...
@property (weak, nonatomic) IBOutlet NSString *firstPlace;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.title = NSLocalizedString(@"Foursquare", @"foursquare");
    }
    return self;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.row == 0){
        self.firstPlace = cell.detailTextLabel.text;
    }
    return cell;
}
...
-(void)userDidSelectVenue{
    self.confirmCallback(self, self.selected.name);
    return;
